...
OUTPUT_CSV = os.path.join(DATA_DIR, "resultados_tictactoe_sft.csv")
...
def llamar_modelo(model_id: str, prompt: str) -> str:
    """
    Envía el prompt al modelo y devuelve el texto de respuesta.
...
def ejecutar_torneo(modelos: Dict[str, str], archivo_salida: str, include_extra_columns: bool) -> None:
    """
    Ejecuta un minitorneo round-robin con 10 partidas como X y 10 como O
    por cada modelo oponente y escribe los resultados en un archivo CSV.
    """
    ...
    with open(archivo_salida, "w", newline='', encoding="utf-8") as f_out:
        ...
        for nombre_oponente, modelo_oponente in modelos.items():
            ...
            print(f"\n▶ Llama-3.2-3B vs {nombre_oponente}")

            # ---- 10 partidas: BASE como X ----
            for i in range(1, n_partidas + 1):
                print(f"  Partida {i}/{n_partidas} → Llama-3.2-3B (X) vs {nombre_oponente} (O)")
            ...
            for i in range(1, n_partidas + 1):
                print(f"  Partida {i}/{n_partidas} → {nombre_oponente} (X) vs Llama-3.2-3B (O)")
